...
// <--- НОВОЕ: Переменная для отображения статуса загрузки или ошибки --->
std::string g_NetworkStatus = "Waiting for start...";
...
std::vector<TradingPair> GetFilteredPairs() {
    ...
    for (const auto& p : source) {
        if (p.symbol.find(queryUpper) != std::string::npos) {
...
// --- СЕТЕВОЙ ПОТОК (С ДИАГНОСТИКОЙ) ---
void NetworkThreadFunc() {
    try {
        g_NetworkStatus = "Connecting to Binance...";
        RestClient restClient;

        g_NetworkStatus = "Downloading ExchangeInfo...";
        // Загрузка справочников (блокирующий вызов)
        restClient.LoadExchangeInfo(g_SharedState->marketData);

        if (g_SharedState->marketData.isLoaded) {
            g_NetworkStatus = "Data Loaded!";
            UpdateSymbolInfo(g_CurrentSymbol, g_IsSpotMode);
        }
        else {
            g_NetworkStatus = "Error: Data Empty (Check logs)";
            g_HasNetworkError = true;
        }
    }
    catch (const std::exception& e) {
        // <--- ВАЖНО: Ловим ошибку и сохраняем её для отображения в UI
        g_NetworkStatus = "Error: " + std::string(e.what());
        g_HasNetworkError = true;
        OutputDebugStringA("[TigerBot] Network Error: ");
...
void HandleHeaderClick(int x, int y) {
    if (x >= 20 && x <= 70) {
        if (!g_IsSpotMode) {
            g_IsSpotMode = true; g_ConnectionDirty = true;
            g_IsDropdownOpen = false; g_SearchQuery = "";
            g_CurrentSymbol = "BTCUSDT"; UpdateSymbolInfo(g_CurrentSymbol, g_IsSpotMode);
        }
    }
    else if (x >= 80 && x <= 160) {
        if (g_IsSpotMode) {
            g_IsSpotMode = false; g_ConnectionDirty = true;
            g_IsDropdownOpen = false; g_SearchQuery = "";
            g_CurrentSymbol = "BTCUSDT"; UpdateSymbolInfo(g_CurrentSymbol, g_IsSpotMode);
        }
    }
    else if (x >= 200 && x <= 350) {
        g_IsDropdownOpen = !g_IsDropdownOpen;
        if (g_IsDropdownOpen) g_SearchQuery = "";
    }
}

void HandleDropdownClick(int x, int y) {
    // Если данные еще грузятся или ошибка - клик игнорируем
    ...
    if (y > dropY && y < dropY + searchH) return; // Поиск

    float listStartY = dropY + searchH;
    float relY = (float)y - listStartY;
    if (relY < 0) return;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_LBUTTONDOWN: {
        ...
        if (g_IsDropdownOpen) {
            // Если меню открыто - клик в него имеет приоритет
            if (y > 40 && x >= 200 && x <= 400) {
        ...
        // Клик в шапку (если не перехвачен выпадающим списком)
        if (y < 50) {
    ...
    case WM_CHAR: {
        if (g_IsDropdownOpen) {
            char c = (char)wParam;
            if (c == VK_BACK) {
                if (!g_SearchQuery.empty()) g_SearchQuery.pop_back();
            }
    ...
    case WM_DESTROY:
        g_Running = false; PostQuitMessage(0); std::exit(0); return 0;
    }
...
int APIENTRY WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nCmdShow) {
    ...
    while (msg.message != WM_QUIT) {
        if (PeekMessage(&msg, NULL, 0U, 0U, PM_REMOVE)) {
            ...
            float headerH = 50.0f;
            float centerY = headerH + (screenH - headerH) / 2.0f;
            ...
            if (g_SmoothCenterPrice > 0) {
                auto DrawLevel = [&](const std::pair<double, double>& lvl, bool isAsk) {
                    ...
                    float r = isAsk ? COL_SELL_R : COL_BUY_R;
                    float g = isAsk ? COL_SELL_G : COL_BUY_G;
                    float b = isAsk ? COL_SELL_B : COL_BUY_B;
            ...
            if (g_IsDropdownOpen) {
                ...
                // <--- ДИАГНОСТИКА: Показываем статус загрузки или ошибку
                if (!g_SharedState->marketData.isLoaded) {
                    // Рисуем красным, если ошибка, или обычным, если просто грузится
                    float r = g_HasNetworkError ? 0.8f : 0.15f;
                    float g = g_HasNetworkError ? 0.1f : 0.15f;
                    float b = g_HasNetworkError ? 0.1f : 0.15f;
