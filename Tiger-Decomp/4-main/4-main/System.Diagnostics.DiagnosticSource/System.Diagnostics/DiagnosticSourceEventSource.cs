...
internal sealed class DiagnosticSourceEventSource : EventSource
{
	...
	internal sealed class FilterAndTransform
	{
		...
		internal static void AddNewActivitySourceTransform(string filterAndPayloadSpec, int startIdx, int endIdx, DiagnosticSourceEventSource eventSource)
		{
			...
			System.ReadOnlySpan<char> readOnlySpan = MemoryExtensions.Trim(MemoryExtensions.AsSpan(filterAndPayloadSpec, startIdx + "[AS]".Length, ((num >= 0) ? num : endIdx) - startIdx - "[AS]".Length));
			...
			if (num2 >= 0)
			{
				...
				System.ReadOnlySpan<char> readOnlySpan3 = MemoryExtensions.Trim(readOnlySpan.Slice(num2 + 1));
				int num3 = MemoryExtensions.IndexOf<char>(readOnlySpan3, '-');
				System.ReadOnlySpan<char> readOnlySpan4;
				if (num3 >= 0)
				{
					readOnlySpan4 = MemoryExtensions.Trim(readOnlySpan3.Slice(0, num3));
					readOnlySpan3 = MemoryExtensions.Trim(readOnlySpan3.Slice(num3 + 1));
			...
			if (num4 >= 0)
			{
				activityName = MemoryExtensions.Trim(readOnlySpan2.Slice(num4 + 1)).ToString();
				readOnlySpan2 = MemoryExtensions.Trim(readOnlySpan2.Slice(0, num4));
			}
			new FilterAndTransform(filterAndPayloadSpec, endIdx, num, readOnlySpan2.ToString(), activityName, events, samplingResult, eventSource);
		}
