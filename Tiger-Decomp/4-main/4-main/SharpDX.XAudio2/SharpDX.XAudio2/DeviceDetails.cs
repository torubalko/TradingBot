...
public struct DeviceDetails
{
	...
	{
		fixed (char* deviceID = &@ref.DeviceID)
		{
			DeviceID = Utilities.PtrToStringUni((IntPtr)deviceID, 256);
		}
		fixed (char* displayName = &@ref.DisplayName)
		{
			DisplayName = Utilities.PtrToStringUni((IntPtr)displayName, 256);
		}
		Role = @ref.Role;
		OutputFormat = new WaveFormatExtensible();
		OutputFormat.__MarshalFrom(ref @ref.OutputFormat);
	}

	internal unsafe void __MarshalTo(ref __Native @ref)
	{
		fixed (char* deviceID = DeviceID)
		{
			fixed (char* deviceID2 = &@ref.DeviceID)
			{
				Utilities.CopyMemory((IntPtr)deviceID2, (IntPtr)deviceID, DeviceID.Length * 2);
			}
		}
		fixed (char* displayName = DisplayName)
		{
			fixed (char* deviceID2 = &@ref.DisplayName)
			{
				Utilities.CopyMemory((IntPtr)deviceID2, (IntPtr)displayName, DisplayName.Length * 2);
			}
		}
		@ref.Role = Role;
