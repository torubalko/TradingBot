...
public class Pkcs8EncryptedPrivateKeyInfo
{
	...
	public PrivateKeyInfo DecryptPrivateKeyInfo(IDecryptorBuilderProvider inputDecryptorProvider)
	{
		try
		{
			ICipher cipher = inputDecryptorProvider.CreateDecryptorBuilder(encryptedPrivateKeyInfo.EncryptionAlgorithm).BuildCipher(new MemoryInputStream(encryptedPrivateKeyInfo.GetEncryptedData()));
			Stream strm = cipher.Stream;
