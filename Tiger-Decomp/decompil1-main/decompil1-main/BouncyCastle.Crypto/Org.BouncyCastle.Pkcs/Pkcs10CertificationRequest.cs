...
public class Pkcs10CertificationRequest : CertificationRequest
{
	...
	internal static string GetSignatureName(AlgorithmIdentifier sigAlgId)
	{
		...
		if (asn1Params != null && !(asn1Params is Asn1Null) && sigAlgId.Algorithm.Equals(PkcsObjectIdentifiers.IdRsassaPss))
		{
			return GetDigestAlgName(RsassaPssParameters.GetInstance(asn1Params).HashAlgorithm.Algorithm) + "withRSAandMGF1";
		}
	...
	public X509Extensions GetRequestedExtensions()
	{
		if (reqInfo.Attributes != null)
		{
			foreach (Asn1Encodable item in reqInfo.Attributes)
			{
				...
				foreach (Asn1Encodable item2 in Asn1Sequence.GetInstance(attributePkcs.AttrValues[0]))
				{
					Asn1Sequence itemSeq = Asn1Sequence.GetInstance(item2);
					if (itemSeq.Count == 2)
					{
						generator.AddExtension(DerObjectIdentifier.GetInstance(itemSeq[0]), critical: false, Asn1OctetString.GetInstance(itemSeq[1]).GetOctets());
						continue;
					}
					if (itemSeq.Count == 3)
					{
