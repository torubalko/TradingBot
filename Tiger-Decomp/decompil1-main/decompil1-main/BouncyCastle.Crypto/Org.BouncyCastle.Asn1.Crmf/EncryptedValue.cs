...
public class EncryptedValue : Asn1Encodable
{
	...
	private EncryptedValue(Asn1Sequence seq)
	{
		...
		for (index = 0; seq[index] is Asn1TaggedObject; index++)
		{
			...
			{
			case 0:
				intendedAlg = AlgorithmIdentifier.GetInstance(tObj, explicitly: false);
				break;
			case 1:
				symmAlg = AlgorithmIdentifier.GetInstance(tObj, explicitly: false);
				break;
			case 2:
				encSymmKey = DerBitString.GetInstance(tObj, isExplicit: false);
				break;
			case 3:
				keyAlg = AlgorithmIdentifier.GetInstance(tObj, explicitly: false);
				break;
			case 4:
				valueHint = Asn1OctetString.GetInstance(tObj, isExplicit: false);
				break;
			}
		}
