...
public class CertTemplate : Asn1Encodable
{
	...
	private CertTemplate(Asn1Sequence seq)
	{
		...
		foreach (Asn1TaggedObject tObj in seq)
		{
			...
			{
			case 0:
				version = DerInteger.GetInstance(tObj, isExplicit: false);
				break;
			case 1:
				serialNumber = DerInteger.GetInstance(tObj, isExplicit: false);
				break;
			case 2:
				signingAlg = AlgorithmIdentifier.GetInstance(tObj, explicitly: false);
				break;
			case 3:
				issuer = X509Name.GetInstance(tObj, explicitly: true);
				break;
			case 4:
				validity = OptionalValidity.GetInstance(Asn1Sequence.GetInstance(tObj, explicitly: false));
				break;
			case 5:
				subject = X509Name.GetInstance(tObj, explicitly: true);
				break;
			case 6:
				publicKey = SubjectPublicKeyInfo.GetInstance(tObj, explicitly: false);
				break;
			case 7:
				issuerUID = DerBitString.GetInstance(tObj, isExplicit: false);
				break;
			case 8:
				subjectUID = DerBitString.GetInstance(tObj, isExplicit: false);
				break;
			case 9:
				extensions = X509Extensions.GetInstance(tObj, explicitly: false);
				break;
			default:
