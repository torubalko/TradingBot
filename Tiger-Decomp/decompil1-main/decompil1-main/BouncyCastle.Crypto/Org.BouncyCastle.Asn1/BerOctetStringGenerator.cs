...
public class BerOctetStringGenerator : BerGenerator
{
	private class BufferedBerOctetStream : BaseOutputStream
	{
		...
		internal BufferedBerOctetStream(BerOctetStringGenerator gen, byte[] buf)
		{
			...
			_buf = buf;
			_off = 0;
			_derOut = Asn1OutputStream.Create(_gen.Out, "DER");
		}
		...
		public override void WriteByte(byte b)
		{
			...
			if (_off == _buf.Length)
			{
				DerOctetString.Encode(_derOut, withID: true, _buf, 0, _off);
				_off = 0;
			}
		}
		...
		public override void Write(byte[] b, int off, int len)
		{
			...
			if (_off > 0)
			{
				...
				count += available;
				DerOctetString.Encode(_derOut, withID: true, _buf, 0, bufLen);
			}
			int remaining;
			while ((remaining = len - count) >= bufLen)
			{
				DerOctetString.Encode(_derOut, withID: true, b, off + count, bufLen);
				count += bufLen;
			}
