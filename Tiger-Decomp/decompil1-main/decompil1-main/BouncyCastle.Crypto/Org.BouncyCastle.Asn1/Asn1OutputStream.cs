...
public class Asn1OutputStream : DerOutputStream
{
	...
	public override void WriteObject(Asn1Encodable encodable)
	{
		...
		WritePrimitive(encodable.ToAsn1Object(), withID: true);
		FlushInternal();
	}
	...
	public override void WriteObject(Asn1Object primitive)
	{
		...
		WritePrimitive(primitive, withID: true);
		FlushInternal();
	}
	...
	{
		int i = 0;
		for (int count = elements.Length; i < count; i++)
		{
			elements[i].ToAsn1Object().Encode(this, withID: true);
		}
	}
	...
	{
		int i = 0;
		for (int count = primitives.Length; i < count; i++)
		{
			WritePrimitive(primitives[i], withID: true);
		}
	}
