...
internal abstract class Nat384
{
	public static void Mul(uint[] x, uint[] y, uint[] zz)
	{
		...
		uint c19 = c18 + Nat192.AddTo(zz, 0, zz, 6, 0u);
		c18 += Nat192.AddTo(zz, 18, zz, 12, c19);
		uint[] dx = Nat192.Create();
		uint[] dy = Nat192.Create();
		bool neg = Nat192.Diff(x, 6, x, 0, dx, 0) != Nat192.Diff(y, 6, y, 0, dy, 0);
		uint[] tt = Nat192.CreateExt();
		Nat192.Mul(dx, dy, tt);
		c18 += (uint)(neg ? ((int)Nat.AddTo(12, tt, 0, zz, 6)) : Nat.SubFrom(12, tt, 0, zz, 6));
