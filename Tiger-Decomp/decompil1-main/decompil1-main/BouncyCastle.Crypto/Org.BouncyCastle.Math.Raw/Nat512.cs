...
internal abstract class Nat512
{
	public static void Mul(uint[] x, uint[] y, uint[] zz)
	{
		...
		uint c25 = c24 + Nat256.AddTo(zz, 0, zz, 8, 0u);
		c24 += Nat256.AddTo(zz, 24, zz, 16, c25);
		uint[] dx = Nat256.Create();
		uint[] dy = Nat256.Create();
		bool neg = Nat256.Diff(x, 8, x, 0, dx, 0) != Nat256.Diff(y, 8, y, 0, dy, 0);
		uint[] tt = Nat256.CreateExt();
		Nat256.Mul(dx, dy, tt);
		c24 += (uint)(neg ? ((int)Nat.AddTo(16, tt, 0, zz, 8)) : Nat.SubFrom(16, tt, 0, zz, 8));
