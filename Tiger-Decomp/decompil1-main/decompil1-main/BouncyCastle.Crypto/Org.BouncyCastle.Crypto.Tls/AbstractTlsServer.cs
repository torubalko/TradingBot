...
public abstract class AbstractTlsServer : AbstractTlsPeer, TlsServer, TlsPeer
{
	...
	{
		IList sigAlgs = TlsUtilities.GetUsableSignatureAlgorithms(mSupportedSignatureAlgorithms);
		...
		{
			if (Arrays.Contains(mOfferedCipherSuites, cipherSuite) && (eccCipherSuitesEnabled || !TlsEccUtilities.IsEccCipherSuite(cipherSuite)) && TlsUtilities.IsValidCipherSuiteForVersion(cipherSuite, mServerVersion) && TlsUtilities.IsValidCipherSuiteForSignatureAlgorithms(cipherSuite, sigAlgs))
			{
