...
public class SmimeCapabilities : Asn1Encodable
{
	...
	private void DoGetCapabilitiesForOid(DerObjectIdentifier capability, IList list)
	{
		if (capability == null)
		{
			...
			{
				SmimeCapability cap = SmimeCapability.GetInstance(capability2);
				list.Add(cap);
			}
			return;
		}
		...
		{
			SmimeCapability cap2 = SmimeCapability.GetInstance(capability3);
			if (capability.Equals(cap2.CapabilityID))
			{
				list.Add(cap2);
			}
		}
