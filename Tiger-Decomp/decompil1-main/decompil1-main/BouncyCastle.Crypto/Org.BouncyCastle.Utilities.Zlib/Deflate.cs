...
public sealed class Deflate
{
	...
	internal bool _tr_tally(int dist, int lc)
	{
		...
		if ((last_lit & 0x1FFF) == 0 && level > 2)
		{
			...
			{
				out_length += (int)(dyn_dtree[dcode * 2] * (5L + (long)Tree.extra_dbits[dcode]));
			}
			out_length >>= 3;
	...
	internal void set_data_type()
	{
		...
		data_type = (byte)((bin_freq <= ascii_freq >> 2) ? 1u : 0u);
	}
	...
	{
		int max_block_size = System.Math.Min(65535, pending_buf.Length - 5);
		while (true)
		{
			if (lookahead <= 1)
			{
	...
	internal int deflate_fast(int flush)
	{
		...
		while (true)
		{
			...
			if (hash_head != 0L && ((strstart - hash_head) & 0xFFFF) <= w_size - 262 && strategy != 2)
			{
	...
	internal int deflate_slow(int flush)
	{
		...
		while (true)
		{
			...
			if (prev_length >= 3 && match_length <= prev_length)
			{
				...
				bool bflush = _tr_tally(strstart - 1 - prev_match, prev_length - 3);
	...
	internal int longest_match(int cur_match)
	{
		...
		int limit = ((strstart > w_size - 262) ? (strstart - (w_size - 262)) : 0);
