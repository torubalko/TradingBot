...
public class SM2KeyExchange
{
	...
	public virtual byte[][] CalculateKeyWithConfirmation(int kLen, byte[] confirmationTag, ICipherParameters pubParam)
	{
		...
		rv = Kdf(U, zb, za, kLen);
		byte[] inner2 = CalculateInnerHash(mDigest, U, zb, za, otherPub.EphemeralPublicKey.Q, mEphemeralPubPoint);
		return new byte[3][]
		{
			rv,
			S1(mDigest, U, inner2),
	...
	protected virtual ECPoint CalculateU(SM2KeyExchangePublicParameters otherPub)
	{
		...
		ECPoint p1 = ECAlgorithms.CleanPoint(parameters.Curve, otherPub.StaticPublicKey.Q);
		ECPoint p2 = ECAlgorithms.CleanPoint(parameters.Curve, otherPub.EphemeralPublicKey.Q);
	...
	protected virtual byte[] Kdf(ECPoint u, byte[] za, byte[] zb, int klen)
	{
		...
		for (; off < rv.Length; off += copyLen)
		{
			...
			copyLen = System.Math.Min(digestSize, rv.Length - off);
