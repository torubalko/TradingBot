...
public class PgpKeyRingGenerator
{
	...
	public void AddSubKey(PgpKeyPair keyPair, PgpSignatureSubpacketVector hashedPackets, PgpSignatureSubpacketVector unhashedPackets, HashAlgorithmTag hashAlgorithm)
	{
		try
		{
			...
			sGen.InitSign(24, masterKey.PrivateKey);
			...
			keys.Add(new PgpSecretKey(keyPair.PrivateKey, new PgpPublicKey(keyPair.PublicKey, null, subSigs), encAlgorithm, rawPassPhrase, clearPassPhrase: false, useSha1, rand, isMasterKey: false));
		}
		catch (PgpException)
		{
			throw;
	...
	public void AddSubKey(PgpKeyPair keyPair, PgpSignatureSubpacketVector hashedPackets, PgpSignatureSubpacketVector unhashedPackets, HashAlgorithmTag hashAlgorithm, HashAlgorithmTag primaryKeyBindingHashAlgorithm)
	{
		try
		{
			...
			pGen.InitSign(25, keyPair.PrivateKey);
			PgpSignatureSubpacketGenerator spGen = new PgpSignatureSubpacketGenerator(hashedPackets);
			spGen.SetEmbeddedSignature(isCritical: false, pGen.GenerateCertification(masterKey.PublicKey, keyPair.PublicKey));
