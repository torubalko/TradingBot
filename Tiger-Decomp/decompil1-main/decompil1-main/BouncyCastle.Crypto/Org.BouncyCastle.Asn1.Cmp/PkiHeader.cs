...
public class PkiHeader : Asn1Encodable
{
	...
	private PkiHeader(Asn1Sequence seq)
	{
		...
		for (int pos = 3; pos < seq.Count; pos++)
		{
			...
			{
			case 0:
				messageTime = DerGeneralizedTime.GetInstance(tObj, isExplicit: true);
				break;
			case 1:
				protectionAlg = AlgorithmIdentifier.GetInstance(tObj, explicitly: true);
				break;
			case 2:
				senderKID = Asn1OctetString.GetInstance(tObj, isExplicit: true);
				break;
			case 3:
				recipKID = Asn1OctetString.GetInstance(tObj, isExplicit: true);
				break;
			case 4:
				transactionID = Asn1OctetString.GetInstance(tObj, isExplicit: true);
				break;
			case 5:
				senderNonce = Asn1OctetString.GetInstance(tObj, isExplicit: true);
				break;
			case 6:
				recipNonce = Asn1OctetString.GetInstance(tObj, isExplicit: true);
				break;
			case 7:
				freeText = PkiFreeText.GetInstance(tObj, isExplicit: true);
				break;
			case 8:
				generalInfo = Asn1Sequence.GetInstance(tObj, explicitly: true);
				break;
			default:
