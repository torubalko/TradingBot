...
internal class LongArray
{
	...
	public BigInteger ToBigInteger()
	{
		...
		byte[] barr = new byte[8 * (usedLen - 1) + barrI];
		for (int i = 0; i < barrI; i++)
		{
			barr[i] = temp[i];
		}
	...
	public LongArray AddOne()
	{
		...
		long[] array = ResizedInts(resultLen);
		array[0] ^= 1L;
	...
	public LongArray ModMultiplyLD(LongArray other, int m, int[] ks)
	{
		...
		for (int k = 56; k >= 0; k -= 8)
		{
			for (int j = 1; j < aLen; j += 2)
			{
				...
				int u = num2 & MASK;
				int v = (num2 >>> 4) & MASK;
				AddBoth(c1, j - 1, T0, ti[u], T1, ti[v], bMax);
			}
			ShiftUp(c1, 0, cLen, 8);
		}
		for (int k2 = 56; k2 >= 0; k2 -= 8)
		{
			for (int l = 0; l < aLen; l += 2)
			{
				int num3 = (int)(a1[l] >>> k2);
				int u2 = num3 & MASK;
				int v2 = (num3 >>> 4) & MASK;
				AddBoth(c1, l, T0, ti[u2], T1, ti[v2], bMax);
			}
			if (k2 > 0)
			{
				ShiftUp(c1, 0, cLen, 8);
		...
		return ReduceResult(c1, 0, cLen, m, ks);
	}
	...
	public LongArray ModMultiply(LongArray other, int m, int[] ks)
	{
		...
		for (int aPos = 0; aPos < aLen; aPos++)
		{
			...
			while (true)
			{
				...
				aVal >>>= 4;
				int v = (int)aVal & MASK;
				AddBoth(c1, cOff, T0, ti[u], T1, ti[v], bMax);
				aVal >>>= 4;
				if (aVal == 0L)
				{
					break;
		...
		while ((cOff2 -= cLen) != 0)
		{
			AddShiftedUp(c1, cOff2 - cLen, c1, cOff2, cLen, 8);
		}
		return ReduceResult(c1, 0, cLen, m, ks);
	}
	...
	public LongArray ModMultiplyAlt(LongArray other, int m, int[] ks)
	{
		...
		int cTotal = (ci[1] = (ci[0] = aLen) + bTotal);
		for (int i = 2; i < ci.Length; i++)
		{
			cTotal = (ci[i] = cTotal + cLen);
		}
		cTotal += cLen;
		cTotal++;
		long[] c1 = new long[cTotal];
		Interleave(A.m_ints, 0, c1, 0, aLen, width);
		int bOff = aLen;
		Array.Copy(B.m_ints, 0, c1, bOff, bLen);
		for (int bank = 1; bank < banks; bank++)
		{
			ShiftUp(c1, aLen, c1, bOff += bMax, bMax, bank);
		}
		int MASK = (1 << width) - 1;
		int k = 0;
		while (true)
		{
			...
			do
			{
				...
				while (true)
				{
					...
					if (index != 0)
					{
						Add(c1, aPos + ci[index], c1, bOff2, bMax);
					}
					if (++bank2 == banks)
					{
						break;
			...
			ShiftUp(c1, aLen, bTotal, banks);
		}
		int ciPos = ci.Length;
		while (--ciPos > 1)
		{
			if (((ulong)ciPos & 1uL) == 0L)
			{
				AddShiftedUp(c1, ci[ciPos >>> 1], c1, ci[ciPos], cLen, positions);
			}
			else
			{
				Distribute(c1, ci[ciPos], ci[ciPos - 1], ci[1], cLen);
			}
		}
		return ReduceResult(c1, ci[1], cLen, m, ks);
	}
	...
	public LongArray Multiply(LongArray other, int m, int[] ks)
	{
		...
		return new LongArray(c1, 0, cLen);
	}
	...
	private static int ReduceInPlace(long[] buf, int off, int len, int m, int[] ks)
	{
		...
		int numBits = System.Math.Min(len << 6, (m << 1) - 1);
		int excessBits;
	...
	private static long Interleave3_21to63(int x)
	{
		...
		return ((INTERLEAVE3_TABLE[x >>> 14] & 0xFFFFFFFFu) << 42) | ((r21 & 0xFFFFFFFFu) << 21) | (r00 & 0xFFFFFFFFu);
	}
	...
	private static long Interleave3_13to65(int x)
	{
		...
		return ((INTERLEAVE5_TABLE[x >>> 7] & 0xFFFFFFFFu) << 35) | (r00 & 0xFFFFFFFFu);
	}
	...
	private static long Interleave4_16to64(int x)
	{
		...
		return ((INTERLEAVE4_TABLE[x >>> 8] & 0xFFFFFFFFu) << 32) | (r00 & 0xFFFFFFFFu);
	}

	private static long Interleave2_32to64(int x)
	{
		int r00 = INTERLEAVE2_TABLE[x & 0xFF] | (INTERLEAVE2_TABLE[(x >>> 8) & 0xFF] << 16);
		return (((INTERLEAVE2_TABLE[(x >>> 16) & 0xFF] | (INTERLEAVE2_TABLE[x >>> 24] << 16)) & 0xFFFFFFFFu) << 32) | (r00 & 0xFFFFFFFFu);
	}
	...
	public LongArray ModInverse(int m, int[] ks)
	{
		...
		default:
		{
			LongArray uz = Copy();
			int intLen = m + 63 >> 6;
			LongArray vz = new LongArray(intLen);
			ReduceBit(vz.m_ints, 0, m, m, ks);
			LongArray g1z = new LongArray(intLen);
			g1z.m_ints[0] = 1L;
			LongArray g2z = new LongArray(intLen);
			int[] uvDeg = new int[2]
			{
				uzDegree,
				m + 1
			};
			LongArray[] uv = new LongArray[2] { uz, vz };
			int[] ggDeg = new int[2] { 1, 0 };
			LongArray[] gg = new LongArray[2] { g1z, g2z };
			int b = 1;
			int duv1 = uvDeg[b];
			int dgg1 = ggDeg[b];
