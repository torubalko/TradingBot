...
public abstract class TlsExtensionsUtilities
{
	...
	public static byte[] CreateCertificateAuthoritiesExtension(IList authorities)
	{
		...
		{
			TlsUtilities.WriteOpaque16(authority.GetEncoded("DER"), buf);
		}
		return PatchOpaque16(buf);
	}
	...
	public static byte[] CreateOidFiltersExtension(IDictionary filters)
	{
		...
		if (filters != null)
		{
			foreach (DerObjectIdentifier certificateExtensionOid in filters.Keys)
			{
				...
				if (certificateExtensionOid == null || certificateExtensionValues == null)
				{
					throw new TlsFatalAlert(80);
				}
				TlsUtilities.WriteOpaque8(certificateExtensionOid.GetEncoded("DER"), buf);
	...
	public static IList ReadAlpnExtensionClient(byte[] extensionData)
	{
		...
		if (TlsUtilities.ReadUint16(buf) != extensionData.Length - 2)
		{
	...
	public static IList ReadCertificateAuthoritiesExtension(byte[] extensionData)
	{
		...
		while (buf.Position < buf.Length)
		{
			Asn1Object asn1 = TlsUtilities.ReadDerObject(TlsUtilities.ReadOpaque16(buf, 1));
	...
	public static KeyShareEntry ReadKeyShareServerHello(byte[] extensionData)
	{
		...
		MemoryStream memoryStream = new MemoryStream(extensionData, writable: false);
		KeyShareEntry serverShare = KeyShareEntry.Parse(memoryStream);
		TlsProtocol.AssertEmpty(memoryStream);
		return serverShare;
	}
	...
	public static IDictionary ReadOidFiltersExtension(byte[] extensionData)
	{
		...
		while (buf.Position < buf.Length)
		{
			DerObjectIdentifier certificateExtensionOid = DerObjectIdentifier.GetInstance(TlsUtilities.ReadDerObject(TlsUtilities.ReadOpaque8(buf, 1)));
	...
	public static OfferedPsks ReadPreSharedKeyClientHello(byte[] extensionData)
	{
		...
		MemoryStream memoryStream = new MemoryStream(extensionData, writable: false);
		OfferedPsks offeredPsks = OfferedPsks.Parse(memoryStream);
		TlsProtocol.AssertEmpty(memoryStream);
		return offeredPsks;
	}
	...
	public static IList ReadSignatureAlgorithmsExtension(byte[] extensionData)
	{
		...
		MemoryStream memoryStream = new MemoryStream(extensionData, writable: false);
		IList supported_signature_algorithms = TlsUtilities.ParseSupportedSignatureAlgorithms(memoryStream);
		TlsProtocol.AssertEmpty(memoryStream);
	...
	public static CertificateStatusRequest ReadStatusRequestExtension(byte[] extensionData)
	{
		...
		MemoryStream memoryStream = new MemoryStream(extensionData, writable: false);
		CertificateStatusRequest statusRequest = CertificateStatusRequest.Parse(memoryStream);
		TlsProtocol.AssertEmpty(memoryStream);
	...
	private static byte[] PatchOpaque16(MemoryStream buf)
	{
		...
		TlsUtilities.WriteUint16(i, extensionData, 0);
