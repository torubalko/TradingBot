...
public class TspUtil
{
	...
	public static ICollection GetSignatureTimestamps(SignerInformation signerInfo)
	{
		...
		{
			foreach (Org.BouncyCastle.Asn1.Cms.Attribute item in unsignedAttrs.GetAll(PkcsObjectIdentifiers.IdAASignatureTimeStampToken))
			{
				foreach (Asn1Encodable asn1 in item.AttrValues)
				{
					try
					{
						TimeStampToken timeStampToken = new TimeStampToken(Org.BouncyCastle.Asn1.Cms.ContentInfo.GetInstance(asn1.ToAsn1Object()));
						TimeStampTokenInfo tstInfo = timeStampToken.TimeStampInfo;
						if (!Arrays.ConstantTimeAreEqual(DigestUtilities.CalculateDigest(GetDigestAlgName(tstInfo.MessageImprintAlgOid), signerInfo.GetSignature()), tstInfo.GetMessageImprintDigest()))
						{
	...
	public static void ValidateCertificate(X509Certificate cert)
	{
		...
		try
		{
			ExtendedKeyUsage extKey = ExtendedKeyUsage.GetInstance(Asn1Object.FromByteArray(ext.GetOctets()));
