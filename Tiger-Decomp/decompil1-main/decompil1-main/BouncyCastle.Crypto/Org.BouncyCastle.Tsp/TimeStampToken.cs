...
public class TimeStampToken
{
	...
	public TimeStampToken(CmsSignedData signedData)
	{
		...
		try
		{
			...
			signedContent.Write(bOut);
			tstInfo = new TimeStampTokenInfo(TstInfo.GetInstance(Asn1Object.FromByteArray(bOut.ToArray())));
			Org.BouncyCastle.Asn1.Cms.Attribute attr = tsaSignerInfo.SignedAttributes[PkcsObjectIdentifiers.IdAASigningCertificate];
			if (attr != null)
			{
				...
				{
					SigningCertificate signCert2 = SigningCertificate.GetInstance(attr.AttrValues[0]);
					certID = new CertID(EssCertID.GetInstance(signCert2.GetCerts()[0]));
				}
				return;
			}
	...
	public void Validate(X509Certificate cert)
	{
		try
		{
			...
			if (certID.IssuerSerial != null)
			{
				...
				{
					if (names[i].TagNo == 4 && X509Name.GetInstance(names[i].Name).Equivalent(principal))
					{
						found = true;
						break;
					}
