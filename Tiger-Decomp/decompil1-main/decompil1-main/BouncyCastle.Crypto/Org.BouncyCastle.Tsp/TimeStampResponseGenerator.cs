...
public class TimeStampResponseGenerator
{
	...
	{
		Asn1EncodableVector v = new Asn1EncodableVector(new DerInteger((int)status));
	...
	public TimeStampResponse Generate(TimeStampRequest request, BigInteger serialNumber, DateTimeObject genTime)
	{
		...
		try
		{
			...
			try
			{
				tstTokenContentInfo = ContentInfo.GetInstance(Asn1Object.FromByteArray(tokenGenerator.Generate(request, serialNumber, genTime.Value).ToCmsSignedData().GetEncoded()));
			}
			catch (IOException e)
			{
	...
	public TimeStampResponse GenerateGrantedResponse(TimeStampRequest request, BigInteger serialNumber, DateTimeObject genTime, string statusString, X509Extensions additionalExtensions)
	{
		...
		try
		{
			...
			try
			{
				tstTokenContentInfo = ContentInfo.GetInstance(Asn1Object.FromByteArray(tokenGenerator.Generate(request, serialNumber, genTime.Value, additionalExtensions).ToCmsSignedData().GetEncoded()));
			}
			catch (IOException e)
			{
				throw new TspException("Timestamp token received cannot be converted to ContentInfo", e);
