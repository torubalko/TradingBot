...
public class SignatureSubpacketsParser
{
	...
	public SignatureSubpacket ReadPacket()
	{
		...
		else
		{
			...
			isLongLength = true;
			bodyLen = (input.ReadByte() << 24) | (input.ReadByte() << 16) | (input.ReadByte() << 8) | input.ReadByte();
		}
		...
		bool isCritical = (num & 0x80) != 0;
		SignatureSubpacketTag type = (SignatureSubpacketTag)(num & 0x7F);
		if (bytesRead != data.Length)
		{
			data = type switch
			{
				SignatureSubpacketTag.CreationTime => CheckData(data, 4, bytesRead, "Signature Creation Time"), 
				SignatureSubpacketTag.IssuerKeyId => CheckData(data, 8, bytesRead, "Issuer"), 
				SignatureSubpacketTag.KeyExpireTime => CheckData(data, 4, bytesRead, "Signature Key Expiration Time"), 
				SignatureSubpacketTag.ExpireTime => CheckData(data, 4, bytesRead, "Signature Expiration Time"), 
