...
public class BcpgInputStream : BaseInputStream
{
	...
	public Packet ReadPacket()
	{
		...
		if (num)
		{
			...
			{
				bodyLen = (m_in.ReadByte() << 24) | (m_in.ReadByte() << 16) | (m_in.ReadByte() << 8) | m_in.ReadByte();
			}
			else
			{
				partial = true;
		...
		BcpgInputStream objStream = ((!(bodyLen == 0 && partial)) ? new BcpgInputStream(new BufferedStream(new PartialInputStream(this, partial, bodyLen))) : this);
		switch (tag)
		{
