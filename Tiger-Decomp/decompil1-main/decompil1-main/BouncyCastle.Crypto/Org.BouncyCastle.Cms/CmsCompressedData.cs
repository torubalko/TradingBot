...
public class CmsCompressedData
{
	...
	{
		ZInputStream zIn = new ZInputStream(((Asn1OctetString)CompressedData.GetInstance(contentInfo.Content).EncapContentInfo.Content).GetOctetStream());
		try
		{
	...
	{
		ZInputStream zIn = new ZInputStream(new MemoryStream(((Asn1OctetString)CompressedData.GetInstance(contentInfo.Content).EncapContentInfo.Content).GetOctets(), writable: false));
		try
		{
