...
internal class PasswordRecipientInfoGenerator : RecipientInfoGenerator
{
	...
	public RecipientInfo Generate(KeyParameter contentEncryptionKey, SecureRandom random)
	{
		...
		byte[] iv = new byte[Platform.StartsWith(rfc3211WrapperName, "DESEDE") ? 8 : 16];
		random.NextBytes(iv);
		...
		DerSequence seq = new DerSequence(new DerObjectIdentifier(keyEncryptionKeyOID), new DerOctetString(iv));
		AlgorithmIdentifier keyEncryptionAlgorithm = new AlgorithmIdentifier(PkcsObjectIdentifiers.IdAlgPwriKek, seq);
