...
public class CmsSignedDataGenerator : CmsSignedGenerator
{
	private class SignerInf
	{
		...
		{
			string signatureName = Helper.GetDigestAlgName(digestOID) + "with" + Helper.GetEncryptionAlgName(encOID);
			this.outer = outer;
			sigCalc = new Asn1SignatureFactory(signatureName, key);
		...
		internal SignerInfo ToSignerInfo(DerObjectIdentifier contentType, CmsProcessable content, SecureRandom random)
		{
			...
			string algorithm = digestName + "with" + Helper.GetEncryptionAlgName(encOID);
			byte[] hash;
			...
			Stream sigStr = new BufferedStream(streamCalculator.Stream);
			...
			if (sAttr != null)
			{
				...
				signedAttr.EncodeTo(sigStr, "DER");
			}
			else
			{
			...
			byte[] sigBytes = ((IBlockResult)streamCalculator.GetResult()).Collect();
			...
			AlgorithmIdentifier encAlgId = Helper.GetEncAlgorithmIdentifier(new DerObjectIdentifier(encOID), sigX509Parameters);
	...
	public CmsSignedData Generate(string signedContentType, CmsProcessable content, bool encapsulate)
	{
		...
		foreach (SignerInf signer2 in signerInfs)
		{
			try
			{
				digestAlgs.Add(signer2.DigestAlgorithmID);
				signerInfos.Add(signer2.ToSignerInfo(contentTypeOid, content, rand));
			}
			catch (IOException e)
			{
		...
		if (_crls.Count != 0)
		{
			certrevlist = (base.UseDerForCrls ? CmsUtilities.CreateDerSetFromList(_crls) : CmsUtilities.CreateBerSetFromList(_crls));
		}
