...
public class DefaultSignatureAlgorithmIdentifierFinder
{
	...
	private static AlgorithmIdentifier Generate(string signatureAlgorithm)
	{
		...
		AlgorithmIdentifier sigAlgId = (noParams.Contains(sigOID) ? new AlgorithmIdentifier(sigOID) : ((!_params.Contains(algorithmName)) ? new AlgorithmIdentifier(sigOID, DerNull.Instance) : new AlgorithmIdentifier(sigOID, (Asn1Encodable)_params[algorithmName])));
		...
		{
			_ = ((RsassaPssParameters)sigAlgId.Parameters).HashAlgorithm;
		}
		else
		{
			new AlgorithmIdentifier((DerObjectIdentifier)digestOids[sigOID], DerNull.Instance);
		}
		return sigAlgId;
	}
