...
internal class CmsSignedHelper
{
	...
	internal IX509Store CreateAttributeStore(string type, Asn1Set certSet)
	{
			...
			foreach (Asn1Encodable ae in certSet)
			{
				try
				{
					...
					if (obj is Asn1TaggedObject)
					{
						...
						if (tagged.TagNo == 2)
						{
							certs.Add(new X509V2AttributeCertificate(Asn1Sequence.GetInstance(tagged, explicitly: false).GetEncoded()));
						}
					}
				}
				catch (Exception e)
		...
		try
		{
			return X509StoreFactory.Create("AttributeCertificate/" + type, new X509CollectionStoreParameters(certs));
		}
	...
	internal IX509Store CreateCertificateStore(string type, Asn1Set certSet)
	{
		...
		try
		{
			return X509StoreFactory.Create("Certificate/" + type, new X509CollectionStoreParameters(certs));
		}
	...
	internal IX509Store CreateCrlStore(string type, Asn1Set crlSet)
	{
		...
		try
		{
			return X509StoreFactory.Create("CRL/" + type, new X509CollectionStoreParameters(crls));
		}
		catch (ArgumentException e)
		{
			throw new CmsException("can't setup the X509Store", e);
