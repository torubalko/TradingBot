...
public class V2TbsCertListGenerator
{
	...
	public void AddCrlEntry(DerInteger userCertificate, Time revocationDate, int reason, DerGeneralizedTime invalidityDate)
	{
		...
		if (reason != 0)
		{
			...
			try
			{
				...
				extValues.Add(new X509Extension(critical: false, new DerOctetString(crlReason.GetEncoded())));
			}
		...
		if (invalidityDate != null)
		{
			try
			{
				...
				extValues.Add(new X509Extension(critical: false, new DerOctetString(invalidityDate.GetEncoded())));
			}
	...
	public TbsCertificateList GenerateTbsCertList()
	{
		...
		Asn1EncodableVector v = new Asn1EncodableVector(version, signature, issuer, thisUpdate);
		if (nextUpdate != null)
		{
			v.Add(nextUpdate);
		}
