...
public class RijndaelEngine : IBlockCipher
{
	...
	private long Shift(long r, int shift)
	{
		...
		return (long)(temp | (ulong)(r << BC - shift)) & BC_MASK;
	}
	...
	private long[][] GenerateWorkingKey(byte[] key)
	{
		...
		int t = 0;
		int j2 = 0;
		while (j2 < KC && t < (ROUNDS + 1) * (BC / 8))
		{
			for (int k = 0; k < 4; k++)
			{
				W[t / (BC / 8)][k] |= (long)(tk[k, j2] & 0xFF) << t * 8 % BC;
			}
			j2++;
			t++;
		}
		while (t < (ROUNDS + 1) * (BC / 8))
		{
			...
			else
			{
				for (int num = 1; num < 4; num++)
				{
					...
					{
						tk[num2, num] ^= tk[num2, num - 1];
					}
				}
				for (int num3 = 0; num3 < 4; num3++)
				{
					tk[num3, 4] ^= S[tk[num3, 3] & 0xFF];
				}
				for (int num4 = 5; num4 < KC; num4++)
				{
					for (int num5 = 0; num5 < 4; num5++)
					{
						tk[num5, num4] ^= tk[num5, num4 - 1];
					}
				}
			}
			int j3 = 0;
			while (j3 < KC && t < (ROUNDS + 1) * (BC / 8))
			{
				for (int num6 = 0; num6 < 4; num6++)
				{
					W[t / (BC / 8)][num6] |= (long)(tk[num6, j3] & 0xFF) << t * 8 % BC;
				}
				j3++;
				t++;
			}
