...
public sealed class TnepresEngine : SerpentEngineBase
{
	...
	protected override int[] MakeWorkingKey(byte[] key)
	{
		...
		if (off == 0)
		{
			...
			for (int i = 8; i < 16; i++)
			{
				kPad[i] = SerpentEngineBase.RotateLeft(kPad[i - 8] ^ kPad[i - 5] ^ kPad[i - 3] ^ kPad[i - 1] ^ -1640531527 ^ (i - 8), 11);
			}
			Array.Copy(kPad, 8, w, 0, 8);
			for (int j = 8; j < amount; j++)
			{
				w[j] = SerpentEngineBase.RotateLeft(w[j - 8] ^ w[j - 5] ^ w[j - 3] ^ w[j - 1] ^ -1640531527 ^ j, 11);
			}
			Sb3(w[0], w[1], w[2], w[3]);
			w[0] = X0;
			w[1] = X1;
