...
public class IesEngine
{
	...
	private byte[] DecryptBlock(byte[] in_enc, int inOff, int inLen, byte[] z)
	{
		...
		else
		{
			...
			byte[] Buffer2 = GenerateKdfBytes(kParam, cipherKeySize / 8 + macKeySize / 8);
			cipher.Init(forEncryption: false, new KeyParameter(Buffer2, 0, cipherKeySize / 8));
			M = cipher.DoFinal(in_enc, inOff, inLen);
			macKey = new KeyParameter(Buffer2, cipherKeySize / 8, macKeySize / 8);
		}
	...
	private byte[] EncryptBlock(byte[] input, int inOff, int inLen, byte[] z)
	{
		...
		else
		{
			int cipherKeySize = ((IesWithCipherParameters)param).CipherKeySize;
			byte[] Buffer2 = GenerateKdfBytes(kParam, cipherKeySize / 8 + macKeySize / 8);
			cipher.Init(forEncryption: true, new KeyParameter(Buffer2, 0, cipherKeySize / 8));
	...
	public virtual byte[] ProcessBlock(byte[] input, int inOff, int inLen)
	{
		...
		try
		{
			return forEncryption ? EncryptBlock(input, inOff, inLen, zBytes) : DecryptBlock(input, inOff, inLen, zBytes);
		}
		finally
		{
