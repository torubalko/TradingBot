...
internal class SecT113R1Curve : AbstractF2mCurve
{
	...
	{
		ulong[] table = new ulong[len * 2 * 2];
		int pos = 0;
		for (int i = 0; i < len; i++)
		{
			ECPoint obj = points[off + i];
			Nat128.Copy64(((SecT113FieldElement)obj.RawXCoord).x, 0, table, pos);
			pos += 2;
			Nat128.Copy64(((SecT113FieldElement)obj.RawYCoord).x, 0, table, pos);
			pos += 2;
		}
