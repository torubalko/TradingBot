...
internal class SecT239Field
{
	...
	public static void Sqrt(ulong[] x, ulong[] z)
	{
		...
		ulong c3 = c1 >> 49;
		ulong c4 = (c0 >> 49) | (c1 << 15);
		c1 ^= c0 << 15;
		ulong[] tt = Nat256.CreateExt64();
		int[] shifts = new int[2] { 39, 120 };
	...
	protected static void ImplExpand(ulong[] x, ulong[] z)
	{
		...
		ulong x4 = x[2];
		ulong x5 = x[3];
		z[0] = x2 & 0xFFFFFFFFFFFFFFFL;
		z[1] = ((x2 >> 60) ^ (x3 << 4)) & 0xFFFFFFFFFFFFFFFL;
		z[2] = ((x3 >> 56) ^ (x4 << 8)) & 0xFFFFFFFFFFFFFFFL;
		z[3] = (x4 >> 52) ^ (x5 << 12);
	}
	...
	protected static void ImplMulwAcc(ulong[] u, ulong x, ulong y, ulong[] z, int zOff)
	{
		...
		while ((k -= 6) > 0);
		h ^= (x & 0x820820820820820L & (ulong)((long)(y << 4) >> 63)) >> 5;
		z[zOff] ^= l & 0xFFFFFFFFFFFFFFFL;
		z[zOff + 1] ^= (l >> 60) ^ (h << 4);
	}
