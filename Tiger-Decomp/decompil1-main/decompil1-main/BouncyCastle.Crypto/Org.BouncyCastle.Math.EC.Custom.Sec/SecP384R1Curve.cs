...
internal class SecP384R1Curve : AbstractFpCurve
{
	...
	{
		uint[] table = new uint[len * 12 * 2];
		int pos = 0;
		for (int i = 0; i < len; i++)
		{
			ECPoint p = points[off + i];
			Nat.Copy(12, ((SecP384R1FieldElement)p.RawXCoord).x, 0, table, pos);
			pos += 12;
			Nat.Copy(12, ((SecP384R1FieldElement)p.RawYCoord).x, 0, table, pos);
			pos += 12;
		}
