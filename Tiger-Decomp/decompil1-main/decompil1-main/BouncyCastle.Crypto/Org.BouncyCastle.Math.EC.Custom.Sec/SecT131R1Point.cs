...
internal class SecT131R1Point : AbstractF2mPoint
{
	...
	public override ECPoint Add(ECPoint b)
	{
		...
		if (X2.IsZero)
		{
			...
			if (X3.IsZero)
			{
				return new SecT131R1Point(curve, X3, curve.B.Sqrt(), base.IsCompressed);
			}
		...
		return new SecT131R1Point(curve, X3, L4, new ECFieldElement[1] { Z3 }, base.IsCompressed);
	}
	...
	public override ECPoint Twice()
	{
		...
		if (T.IsZero)
		{
			return new SecT131R1Point(curve, T, curve.B.Sqrt(), base.IsCompressed);
		}
		ECFieldElement X3 = T.Square();
		ECFieldElement Z3 = (Z1IsOne ? T : T.Multiply(Z1Sq));
		ECFieldElement L3 = (Z1IsOne ? X1 : X1.Multiply(Z1)).SquarePlusProduct(T, L1Z1).Add(X3).Add(Z3);
		return new SecT131R1Point(curve, X3, L3, new ECFieldElement[1] { Z3 }, base.IsCompressed);
	}
	...
	public override ECPoint TwicePlus(ECPoint b)
	{
		...
		if (A.IsZero)
		{
			return new SecT131R1Point(curve, A, curve.B.Sqrt(), base.IsCompressed);
		}
		ECFieldElement X3 = A.Square().Multiply(X2Z1Sq);
		ECFieldElement Z4 = A.Multiply(B).Multiply(Z1Sq);
		ECFieldElement L3 = A.Add(B).Square().MultiplyPlusProduct(T, L2plus1, Z4);
		return new SecT131R1Point(curve, X3, L3, new ECFieldElement[1] { Z4 }, base.IsCompressed);
	}
	...
	public override ECPoint Negate()
	{
		...
		ECFieldElement L = base.RawYCoord;
		ECFieldElement Z = base.RawZCoords[0];
		return new SecT131R1Point(Curve, X, L.Add(Z), new ECFieldElement[1] { Z }, base.IsCompressed);
	}
}
