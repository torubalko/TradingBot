...
internal class SecT409R1Curve : AbstractF2mCurve
{
	...
	{
		ulong[] table = new ulong[len * 7 * 2];
		int pos = 0;
		for (int i = 0; i < len; i++)
		{
			ECPoint obj = points[off + i];
			Nat448.Copy64(((SecT409FieldElement)obj.RawXCoord).x, 0, table, pos);
			pos += 7;
			Nat448.Copy64(((SecT409FieldElement)obj.RawYCoord).x, 0, table, pos);
			pos += 7;
		}
