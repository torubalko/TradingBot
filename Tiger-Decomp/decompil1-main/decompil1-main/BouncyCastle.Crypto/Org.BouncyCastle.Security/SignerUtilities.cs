...
public sealed class SignerUtilities
{
	...
	{
		AlgorithmIdentifier hashAlgorithm = new AlgorithmIdentifier(DigestUtilities.GetObjectIdentifier(digestName), DerNull.Instance);
		AlgorithmIdentifier maskGenAlgorithm = new AlgorithmIdentifier(PkcsObjectIdentifiers.IdMgf1, hashAlgorithm);
		int saltLen = DigestUtilities.GetDigest(digestName).GetDigestSize();
		return new RsassaPssParameters(hashAlgorithm, maskGenAlgorithm, new DerInteger(saltLen), new DerInteger(1));
	}
	...
	public static ISigner GetSigner(string algorithm)
	{
		...
		if (Platform.EndsWith(mechanism, "/X9.31"))
		{
			...
			if (withPos > 0)
			{
				...
				IDigest digest6 = DigestUtilities.GetDigest(x931.Substring(0, withPos));
				if (x931.Substring(endPos, x931.Length - endPos).Equals("RSA"))
				{
