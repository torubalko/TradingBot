...
public sealed class CipherUtilities
{
	...
	public static IBufferedCipher GetCipher(string algorithm)
	{
		...
		{
			return new BufferedIesCipher(new IesEngine(iesAgreement, new Kdf2BytesGenerator(new Sha1Digest()), new HMac(new Sha1Digest())));
		}
		...
		string aliased2 = (string)algorithms[algorithmName];
		if (aliased2 != null)
		{
			algorithmName = aliased2;
		}
		CipherAlgorithm cipherAlgorithm;
		try
		{
			cipherAlgorithm = (CipherAlgorithm)(object)Enums.GetEnumValue(typeof(CipherAlgorithm), algorithmName);
		}
		...
		if (parts.Length > 2)
		{
			...
			else
			{
				try
				{
					cipherPadding = (CipherPadding)(object)Enums.GetEnumValue(typeof(CipherPadding), paddingName);
				}
				catch (ArgumentException)
				{
					throw new SecurityUtilityException("Cipher " + algorithm + " not recognised.");
		...
		if (parts.Length > 1)
		{
			...
			try
			{
				switch ((modeName == "") ? CipherMode.NONE : ((CipherMode)(object)Enums.GetEnumValue(typeof(CipherMode), modeName)))
				{
				case CipherMode.CBC:
				...
				case CipherMode.CFB:
				{
					int bits2 = ((di < 0) ? (8 * blockCipher.GetBlockSize()) : int.Parse(mode.Substring(di)));
					blockCipher = new CfbBlockCipher(blockCipher, bits2);
					break;
				}
				case CipherMode.CTR:
