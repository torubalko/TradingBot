...
public sealed class PbeUtilities
{
	...
	public static bool IsPkcs12(string algorithm)
	{
		...
		if (mechanism != null)
		{
			return "Pkcs12".Equals(algorithmType[mechanism]);
		}
		return false;
	}
	...
	public static bool IsPkcs5Scheme1(string algorithm)
	{
		...
		if (mechanism != null)
		{
			return "Pkcs5S1".Equals(algorithmType[mechanism]);
		}
		return false;
	}
	...
	public static bool IsPkcs5Scheme2(string algorithm)
	{
		...
		if (mechanism != null)
		{
			return "Pkcs5S2".Equals(algorithmType[mechanism]);
		}
		return false;
	}
	...
	public static bool IsOpenSsl(string algorithm)
	{
		...
		if (mechanism != null)
		{
			return "OpenSsl".Equals(algorithmType[mechanism]);
		}
		return false;
	}
	...
	public static Asn1Encodable GenerateAlgorithmParameters(DerObjectIdentifier cipherAlgorithm, DerObjectIdentifier hashAlgorithm, byte[] salt, int iterationCount, SecureRandom secureRandom)
	{
		if (NistObjectIdentifiers.IdAes128Cbc.Equals(cipherAlgorithm) || NistObjectIdentifiers.IdAes192Cbc.Equals(cipherAlgorithm) || NistObjectIdentifiers.IdAes256Cbc.Equals(cipherAlgorithm) || NistObjectIdentifiers.IdAes128Cfb.Equals(cipherAlgorithm) || NistObjectIdentifiers.IdAes192Cfb.Equals(cipherAlgorithm) || NistObjectIdentifiers.IdAes256Cfb.Equals(cipherAlgorithm))
		{
			...
			EncryptionScheme encScheme = new EncryptionScheme(cipherAlgorithm, new DerOctetString(iv));
	...
	public static ICipherParameters GenerateCipherParameters(string algorithm, char[] password, bool wrongPkcs12Zero, Asn1Encodable pbeParameters)
	{
		...
		if (IsPkcs5Scheme2(mechanism))
		{
			...
			DerObjectIdentifier encOid = encryptionScheme.Algorithm;
			Asn1Object encParams = encryptionScheme.Parameters.ToAsn1Object();
			Pbkdf2Params pbeParams = Pbkdf2Params.GetInstance(instance3.KeyDerivationFunc.Parameters.ToAsn1Object());
			IDigest digest = DigestUtilities.GetDigest(pbeParams.Prf.Algorithm);
			byte[] iv = ((!encOid.Equals(PkcsObjectIdentifiers.RC2Cbc)) ? Asn1OctetString.GetInstance(encParams).GetOctets() : RC2CbcParameter.GetInstance(encParams).GetIV());
			salt = pbeParams.GetSalt();
			iterationCount = pbeParams.IterationCount.IntValue;
			keyBytes = PbeParametersGenerator.Pkcs5PasswordToBytes(password);
			int keyLength = ((pbeParams.KeyLength != null) ? (pbeParams.KeyLength.IntValue * 8) : GeneratorUtilities.GetDefaultKeySize(encOid));
			parameters = MakePbeGenerator((string)algorithmType[mechanism], digest, keyBytes, salt, iterationCount).GenerateDerivedParameters(encOid.Id, keyLength);
			if (iv != null && !Arrays.AreEqual(iv, new byte[iv.Length]))
			{
		...
		{
			PbeParametersGenerator generator = MakePbeGenerator((string)algorithmType[mechanism], new Sha1Digest(), keyBytes, salt, iterationCount);
		...
		{
			PbeParametersGenerator generator2 = MakePbeGenerator((string)algorithmType[mechanism], new Sha256Digest(), keyBytes, salt, iterationCount);
			if (mechanism.Equals("PBEwithSHA-256and128bitAES-CBC-BC"))
			{
				parameters = generator2.GenerateDerivedParameters("AES", 128, 128);
			}
			else if (mechanism.Equals("PBEwithSHA-256and192bitAES-CBC-BC"))
			{
				parameters = generator2.GenerateDerivedParameters("AES", 192, 128);
			}
			else if (mechanism.Equals("PBEwithSHA-256and256bitAES-CBC-BC"))
			{
				parameters = generator2.GenerateDerivedParameters("AES", 256, 128);
			}
		}
		else if (Platform.StartsWith(mechanism, "PBEwithMD5"))
		{
			PbeParametersGenerator generator3 = MakePbeGenerator((string)algorithmType[mechanism], new MD5Digest(), keyBytes, salt, iterationCount);
			if (mechanism.Equals("PBEwithMD5andDES-CBC"))
			{
				parameters = generator3.GenerateDerivedParameters("DES", 64, 64);
			}
			else if (mechanism.Equals("PBEwithMD5andRC2-CBC"))
			{
				parameters = generator3.GenerateDerivedParameters("RC2", 64, 64);
			}
			else if (mechanism.Equals("PBEwithMD5and128bitAES-CBC-OpenSSL"))
			{
				parameters = generator3.GenerateDerivedParameters("AES", 128, 128);
			}
			else if (mechanism.Equals("PBEwithMD5and192bitAES-CBC-OpenSSL"))
			{
				parameters = generator3.GenerateDerivedParameters("AES", 192, 128);
			}
			else if (mechanism.Equals("PBEwithMD5and256bitAES-CBC-OpenSSL"))
			{
				parameters = generator3.GenerateDerivedParameters("AES", 256, 128);
			}
		}
		else if (Platform.StartsWith(mechanism, "PBEwithMD2"))
		{
			PbeParametersGenerator generator4 = MakePbeGenerator((string)algorithmType[mechanism], new MD2Digest(), keyBytes, salt, iterationCount);
			if (mechanism.Equals("PBEwithMD2andDES-CBC"))
			{
				parameters = generator4.GenerateDerivedParameters("DES", 64, 64);
			}
			else if (mechanism.Equals("PBEwithMD2andRC2-CBC"))
			{
				parameters = generator4.GenerateDerivedParameters("RC2", 64, 64);
			}
		}
		else if (Platform.StartsWith(mechanism, "PBEwithHmac"))
		{
			IDigest digest2 = DigestUtilities.GetDigest(mechanism.Substring("PBEwithHmac".Length));
			PbeParametersGenerator pbeParametersGenerator = MakePbeGenerator((string)algorithmType[mechanism], digest2, keyBytes, salt, iterationCount);
			int bitLen = digest2.GetDigestSize() * 8;
			parameters = pbeParametersGenerator.GenerateDerivedMacParameters(bitLen);
		}
	...
	public static object CreateEngine(string algorithm)
	{
		...
		if (Platform.StartsWith(mechanism, "PBEwithMD2") || Platform.StartsWith(mechanism, "PBEwithMD5") || Platform.StartsWith(mechanism, "PBEwithSHA-1") || Platform.StartsWith(mechanism, "PBEwithSHA-256"))
		{
