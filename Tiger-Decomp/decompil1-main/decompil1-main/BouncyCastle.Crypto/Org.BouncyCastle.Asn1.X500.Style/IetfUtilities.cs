...
public abstract class IetfUtilities
{
	public static string ValueToString(Asn1Encodable value)
	{
		...
		else
		{
			...
			{
				vBuf.Append('#');
				vBuf.Append(Hex.ToHexString(value.ToAsn1Object().GetEncoded("DER")));
			}
		...
		if (vBuf.Length > 0)
		{
			for (; vBuf.Length > start && vBuf[start] == ' '; start += 2)
			{
				vBuf.Insert(start, "\\");
			}
		}
	...
	public static string Canonicalize(string s)
	{
		...
		{
			int start;
			for (start = 0; start + 1 < value.Length && value[start] == '\\' && value[start + 1] == ' '; start += 2)
			{
			}
