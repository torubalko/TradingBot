...
public class CertificationRequestInfo : Asn1Encodable
{
	...
	private CertificationRequestInfo(Asn1Sequence seq)
	{
		...
		if (seq.Count > 3)
		{
			...
			attributes = Asn1Set.GetInstance(tagobj, explicitly: false);
		}
		ValidateAttributes(attributes);
		if (subject == null || version == null || subjectPKInfo == null)
		{
	...
	private static void ValidateAttributes(Asn1Set attributes)
	{
		...
		{
			AttributePkcs attr = AttributePkcs.GetInstance(attribute.ToAsn1Object());
			if (attr.AttrType.Equals(PkcsObjectIdentifiers.Pkcs9AtChallengePassword) && attr.AttrValues.Count != 1)
			{
