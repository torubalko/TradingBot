...
internal sealed class PemUtilities
{
	...
	private static void ParseDekAlgName(string dekAlgName, out PemBaseAlg baseAlg, out PemMode mode)
	{
		try
		{
			...
			if (pos >= 0)
			{
				baseAlg = (PemBaseAlg)(object)Enums.GetEnumValue(typeof(PemBaseAlg), dekAlgName.Substring(0, pos));
				mode = (PemMode)(object)Enums.GetEnumValue(typeof(PemMode), dekAlgName.Substring(pos + 1));
				return;
			}
		}
	...
	internal static byte[] Crypt(bool encrypt, byte[] bytes, char[] password, string dekAlgName, byte[] iv)
	{
		...
		IBufferedCipher cipher = CipherUtilities.GetCipher(algorithm + "/" + mode.ToString() + "/" + padding);
	...
	private static ICipherParameters GetCipherParameters(char[] password, PemBaseAlg baseAlg, byte[] salt)
	{
		...
		return openSslPbeParametersGenerator.GenerateDerivedParameters(algorithm, keyBits);
	}
}
