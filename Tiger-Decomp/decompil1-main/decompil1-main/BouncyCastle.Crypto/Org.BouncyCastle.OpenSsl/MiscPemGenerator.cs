...
public class MiscPemGenerator : PemObjectGenerator
{
	...
	private static PemObject CreatePemObject(object obj)
	{
		...
		else
		{
			...
			type = "PKCS7";
			encoding = ((Org.BouncyCastle.Asn1.Cms.ContentInfo)obj).GetEncoded();
		}
	...
	private static PemObject CreatePemObject(object obj, string algorithm, char[] password, SecureRandom random)
	{
		...
		byte[] iv = new byte[Platform.StartsWith(dekAlgName, "AES-") ? 16 : 8];
		random.NextBytes(iv);
		byte[] encData = PemUtilities.Crypt(encrypt: true, keyData, password, dekAlgName, iv);
	...
	private static byte[] EncodePrivateKey(AsymmetricKeyParameter akp, out string keyType)
	{
		...
		if (oid.Equals(X9ObjectIdentifiers.IdDsa))
		{
			...
			return new DerSequence(new DerInteger(0), new DerInteger(p.P), new DerInteger(p.Q), new DerInteger(p.G), new DerInteger(y), new DerInteger(x)).GetEncoded();
		}
		...
		if (oid.Equals(CryptoProObjectIdentifiers.GostR3410x2001) || oid.Equals(X9ObjectIdentifiers.IdECPublicKey))
		{
