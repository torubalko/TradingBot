...
public class X9ECParameters : Asn1Encodable
{
	...
	public X9ECParameters(Asn1Sequence seq)
	{
		...
		X9Curve x9c = new X9Curve(X9FieldID.GetInstance(seq[1]), n, h, Asn1Sequence.GetInstance(seq[2]));
		curve = x9c.Curve;
		object p = seq[3];
		if (p is X9ECPoint)
		{
	...
	{
		Asn1EncodableVector v = new Asn1EncodableVector(new DerInteger(BigInteger.One), fieldID, new X9Curve(curve, seed), g, new DerInteger(n));
		if (h != null)
		{
