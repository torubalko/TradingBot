...
public class ECPrivateKeyStructure : Asn1Encodable
{
	...
	public ECPrivateKeyStructure(BigInteger key, DerBitString publicKey, Asn1Encodable parameters)
	{
		...
		Asn1EncodableVector v = new Asn1EncodableVector(new DerInteger(1), new DerOctetString(key.ToByteArrayUnsigned()));
		if (parameters != null)
		{
			v.Add(new DerTaggedObject(explicitly: true, 0, parameters));
		}
		if (publicKey != null)
		{
			v.Add(new DerTaggedObject(explicitly: true, 1, publicKey));
		}
	...
	public ECPrivateKeyStructure(int orderBitLength, BigInteger key, DerBitString publicKey, Asn1Encodable parameters)
	{
		...
		Asn1EncodableVector v = new Asn1EncodableVector(new DerInteger(1), new DerOctetString(bytes));
		if (parameters != null)
		{
			v.Add(new DerTaggedObject(explicitly: true, 0, parameters));
		}
