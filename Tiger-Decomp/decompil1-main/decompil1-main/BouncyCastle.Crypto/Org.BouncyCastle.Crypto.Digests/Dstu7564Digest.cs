...
public class Dstu7564Digest : IDigest, IMemoable
{
	...
	public virtual int DoFinal(byte[] output, int outOff)
	{
		...
		long num = (long)(inputBlocks & 0xFFFFFFFFu) * (long)blockSize + (uint)inputBytes << 3;
		Pack.UInt32_To_LE((uint)num, buf, bufOff);
		bufOff += 4;
		Pack.UInt64_To_LE((ulong)((num >>> 32) + ((long)(inputBlocks >> 32) * (long)blockSize << 3)), buf, bufOff);
		ProcessBlock(buf, 0);
		...
		for (int i = columns - neededColumns; i < columns; i++)
		{
			Pack.UInt64_To_LE(state[i], output, outOff);
			outOff += 8;
		}
		Reset();
		return hashSize;
	...
	protected virtual void ProcessBlock(byte[] input, int inOff)
	{
		...
		{
			state[i] ^= tempState1[i] ^ tempState2[i];
		}
	}
	...
	private void Q(ulong[] s)
	{
		for (int round = 0; round < rounds; round++)
		{
			ulong rc = (ulong)(((long)((columns - 1 << 4) ^ round) << 56) | 0xF0F0F0F0F0F0F3L);
			for (int col = 0; col < columns; col++)
			{
				s[col] += rc;
	...
	{
		ulong x1 = ((c & 0x7F7F7F7F7F7F7F7FL) << 1) ^ (((c & 0x8080808080808080uL) >> 7) * 29);
		ulong u = Rotate(8, c) ^ c;
		u ^= Rotate(16, u);
		u ^= Rotate(48, c);
		ulong v = u ^ c ^ x1;
		v = ((v & 0x3F3F3F3F3F3F3F3FL) << 2) ^ (((v & 0x8080808080808080uL) >> 6) * 29) ^ (((v & 0x4040404040404040L) >> 6) * 29);
	...
	private void ShiftRows(ulong[] s)
	{
		...
		case 8:
		{
			...
			ulong c22 = s[6];
			ulong c23 = s[7];
			ulong d2 = (c16 ^ c20) & 0xFFFFFFFF00000000uL;
			c16 ^= d2;
			c20 ^= d2;
			d2 = (c17 ^ c21) & 0xFFFFFFFF000000L;
			c17 ^= d2;
			c21 ^= d2;
			d2 = (c18 ^ c22) & 0xFFFFFFFF0000L;
			c18 ^= d2;
			c22 ^= d2;
			d2 = (c19 ^ c23) & 0xFFFFFFFF00L;
			c19 ^= d2;
			c23 ^= d2;
			d2 = (c16 ^ c18) & 0xFFFF0000FFFF0000uL;
			c16 ^= d2;
			c18 ^= d2;
			d2 = (c17 ^ c19) & 0xFFFF0000FFFF00L;
			c17 ^= d2;
			c19 ^= d2;
			d2 = (c20 ^ c22) & 0xFFFF0000FFFF0000uL;
			c20 ^= d2;
			c22 ^= d2;
			d2 = (c21 ^ c23) & 0xFFFF0000FFFF00L;
			c21 ^= d2;
			c23 ^= d2;
			d2 = (c16 ^ c17) & 0xFF00FF00FF00FF00uL;
			c16 ^= d2;
			c17 ^= d2;
			d2 = (c18 ^ c19) & 0xFF00FF00FF00FF00uL;
			c18 ^= d2;
			c19 ^= d2;
			d2 = (c20 ^ c21) & 0xFF00FF00FF00FF00uL;
			c20 ^= d2;
			c21 ^= d2;
			d2 = (c22 ^ c23) & 0xFF00FF00FF00FF00uL;
			c22 ^= d2;
			c23 ^= d2;
			s[0] = c16;
			s[1] = c17;
		...
		case 16:
		{
			...
			ulong c14 = s[14];
			ulong c15 = s[15];
			ulong d = (c00 ^ c8) & 0xFF00000000000000uL;
			c00 ^= d;
			c8 ^= d;
			d = (c1 ^ c9) & 0xFF00000000000000uL;
			c1 ^= d;
			c9 ^= d;
			d = (c2 ^ c10) & 0xFFFF000000000000uL;
			c2 ^= d;
			c10 ^= d;
			d = (c3 ^ c11) & 0xFFFFFF0000000000uL;
			c3 ^= d;
			c11 ^= d;
			d = (c4 ^ c12) & 0xFFFFFFFF00000000uL;
			c4 ^= d;
			c12 ^= d;
			d = (c5 ^ c13) & 0xFFFFFFFF000000L;
			c5 ^= d;
			c13 ^= d;
			d = (c6 ^ c14) & 0xFFFFFFFFFF0000L;
			c6 ^= d;
			c14 ^= d;
			d = (c7 ^ c15) & 0xFFFFFFFFFFFF00L;
			c7 ^= d;
			c15 ^= d;
			d = (c00 ^ c4) & 0xFFFFFF00000000L;
			c00 ^= d;
			c4 ^= d;
			d = (c1 ^ c5) & 0xFFFFFFFFFF000000uL;
			c1 ^= d;
			c5 ^= d;
			d = (c2 ^ c6) & 0xFF00FFFFFFFF0000uL;
			c2 ^= d;
			c6 ^= d;
			d = (c3 ^ c7) & 0xFF0000FFFFFFFF00uL;
			c3 ^= d;
			c7 ^= d;
			d = (c8 ^ c12) & 0xFFFFFF00000000L;
			c8 ^= d;
			c12 ^= d;
			d = (c9 ^ c13) & 0xFFFFFFFFFF000000uL;
			c9 ^= d;
			c13 ^= d;
			...
			c5 ^= d;
			c7 ^= d;
			d = (c8 ^ c10) & 0xFFFF0000FFFF0000uL;
			c8 ^= d;
			c10 ^= d;
			d = (c9 ^ c11) & 0xFFFF0000FFFF00L;
			c9 ^= d;
			c11 ^= d;
