...
public sealed class WhirlpoolDigest : IDigest, IMemoable
{
	...
	public WhirlpoolDigest()
	{
		...
		{
			int i = 8 * (r - 1);
			_rc[r] = (C0[i] & -72057594037927936L) ^ (C1[i + 1] & 0xFF000000000000L) ^ (C2[i + 2] & 0xFF0000000000L) ^ (C3[i + 3] & 0xFF00000000L) ^ (C4[i + 4] & 0xFF000000u) ^ (C5[i + 5] & 0xFF0000) ^ (C6[i + 6] & 0xFF00) ^ (C7[i + 7] & 0xFF);
		}
	}
	...
	private void processBlock()
	{
		...
		for (int round = 1; round <= 10; round++)
		{
			...
			{
				_L[j] = 0L;
				_L[j] ^= C0[(int)(_K[j & 7] >> 56) & 0xFF];
				_L[j] ^= C1[(int)(_K[(j - 1) & 7] >> 48) & 0xFF];
				_L[j] ^= C2[(int)(_K[(j - 2) & 7] >> 40) & 0xFF];
				_L[j] ^= C3[(int)(_K[(j - 3) & 7] >> 32) & 0xFF];
				_L[j] ^= C4[(int)(_K[(j - 4) & 7] >> 24) & 0xFF];
				_L[j] ^= C5[(int)(_K[(j - 5) & 7] >> 16) & 0xFF];
				_L[j] ^= C6[(int)(_K[(j - 6) & 7] >> 8) & 0xFF];
				_L[j] ^= C7[(int)_K[(j - 7) & 7] & 0xFF];
			}
			...
			{
				_L[k] = _K[k];
				_L[k] ^= C0[(int)(_state[k & 7] >> 56) & 0xFF];
				_L[k] ^= C1[(int)(_state[(k - 1) & 7] >> 48) & 0xFF];
				_L[k] ^= C2[(int)(_state[(k - 2) & 7] >> 40) & 0xFF];
				_L[k] ^= C3[(int)(_state[(k - 3) & 7] >> 32) & 0xFF];
				_L[k] ^= C4[(int)(_state[(k - 4) & 7] >> 24) & 0xFF];
				_L[k] ^= C5[(int)(_state[(k - 5) & 7] >> 16) & 0xFF];
				_L[k] ^= C6[(int)(_state[(k - 6) & 7] >> 8) & 0xFF];
				_L[k] ^= C7[(int)_state[(k - 7) & 7] & 0xFF];
			}
			Array.Copy(_L, 0, _state, 0, _state.Length);
		}
		for (int l = 0; l < 8; l++)
		{
			_hash[l] ^= _state[l] ^ _block[l];
		}
	}
