...
public class SkeinEngine : IMemoable
{
	...
	private class UbiTweak
	{
		...
		public void AdvancePosition(int advance)
		{
			if (extendedPosition)
			{
				...
				{
					carry = (parts[i] = carry + parts[i]) >> 32;
				}
				tweak[0] = ((parts[1] & 0xFFFFFFFFu) << 32) | (parts[0] & 0xFFFFFFFFu);
				tweak[1] = (tweak[1] & 0xFFFFFFFF00000000uL) | (parts[2] & 0xFFFFFFFFu);
			}
			else
			{
				...
				tweak[0] = position;
				if (position > 18446744069414584320uL)
				{
	...
	public int DoFinal(byte[] outBytes, int outOff)
	{
		...
		{
			int toWrite = System.Math.Min(blockSize, outputSizeBytes - j * blockSize);
			Output((ulong)j, outBytes, outOff + j * blockSize, toWrite);
		}
		Reset();
