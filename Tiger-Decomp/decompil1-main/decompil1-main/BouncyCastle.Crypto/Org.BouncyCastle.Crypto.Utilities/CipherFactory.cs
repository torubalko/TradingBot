...
public class CipherFactory
{
	...
	public static object CreateContentCipher(bool forEncryption, ICipherParameters encKey, AlgorithmIdentifier encryptionAlgID)
	{
		...
		if (encAlg.Equals(PkcsObjectIdentifiers.rc4))
		{
			...
			((IStreamCipher)rC4Engine).Init(forEncryption, encKey);
			return rC4Engine;
		}
		...
		{
			if (encAlg.Equals(PkcsObjectIdentifiers.DesEde3Cbc) || encAlg.Equals(AlgorithmIdentifierFactory.IDEA_CBC) || encAlg.Equals(NistObjectIdentifiers.IdAes128Cbc) || encAlg.Equals(NistObjectIdentifiers.IdAes192Cbc) || encAlg.Equals(NistObjectIdentifiers.IdAes256Cbc) || encAlg.Equals(NttObjectIdentifiers.IdCamellia128Cbc) || encAlg.Equals(NttObjectIdentifiers.IdCamellia192Cbc) || encAlg.Equals(NttObjectIdentifiers.IdCamellia256Cbc) || encAlg.Equals(KisaObjectIdentifiers.IdSeedCbc) || encAlg.Equals(OiwObjectIdentifiers.DesCbc))
			{
			...
			{
				if (!encAlg.Equals(PkcsObjectIdentifiers.RC2Cbc))
				{
					throw new InvalidOperationException("cannot match parameters");
				}
				RC2CbcParameter cbcParams2 = RC2CbcParameter.GetInstance(sParams);
				cipher.Init(forEncryption, new ParametersWithIV(new RC2Parameters(((KeyParameter)encKey).GetKey(), rc2Ekb[cbcParams2.RC2ParameterVersion.IntValue]), cbcParams2.GetIV()));
			}
		}
		else if (encAlg.Equals(PkcsObjectIdentifiers.DesEde3Cbc) || encAlg.Equals(AlgorithmIdentifierFactory.IDEA_CBC) || encAlg.Equals(AlgorithmIdentifierFactory.CAST5_CBC))
		{
