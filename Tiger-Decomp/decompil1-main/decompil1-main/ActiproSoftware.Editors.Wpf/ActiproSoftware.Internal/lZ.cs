...
internal class lZ : C6<int?>, ISpinnablePart<Color?>, IPart
{
	internal static lZ yjg;
	...
	public bool ApplyIncrementalChange(IncrementalChangeRequest<Color?> P_0)
	{
		...
		case IncrementalChangeRequestKind.Decrease:
			P_0.Value = VdT.UbT(P_0.Value.Value, -1, P_0.SpinWrapping);
			break;
		case IncrementalChangeRequestKind.Increase:
			P_0.Value = VdT.UbT(P_0.Value.Value, 1, P_0.SpinWrapping);
			break;
		case IncrementalChangeRequestKind.MultipleDecrease:
			P_0.Value = VdT.UbT(P_0.Value.Value, -16, P_0.SpinWrapping);
			break;
	...
	internal static bool sjs()
	{
		return yjg == null;
	}

	internal static lZ KjY()
	{
		return yjg;
	}
}
