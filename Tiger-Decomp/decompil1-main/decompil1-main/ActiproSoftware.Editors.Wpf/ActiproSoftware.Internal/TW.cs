...
internal class TW : C6<short?>, ISpinnablePart<short?>, IPart
{
	private static TW zJj;

	public bool ApplyIncrementalChange(IncrementalChangeRequest<short?> P_0)
	{
		if (P_0 == null)
		{
		...
		short num = P_0.SmallChange ?? 1;
		short num2 = P_0.LargeChange ?? 5;
		switch (P_0.Kind)
		{
		case IncrementalChangeRequestKind.Decrease:
			if (num != 0)
			{
				P_0.Value = YdV.EDp(P_0.Value.Value, -num, P_0.Minimum, P_0.Maximum, P_0.SpinWrapping);
			}
			break;
		...
		case IncrementalChangeRequestKind.MultipleDecrease:
			if (num2 != 0)
			{
				P_0.Value = YdV.EDp(P_0.Value.Value, -num2, P_0.Minimum, P_0.Maximum, P_0.SpinWrapping);
			}
			break;
	...
	internal static bool jJG()
	{
		return zJj == null;
	}

	internal static TW XJJ()
	{
		return zJj;
	}
}
