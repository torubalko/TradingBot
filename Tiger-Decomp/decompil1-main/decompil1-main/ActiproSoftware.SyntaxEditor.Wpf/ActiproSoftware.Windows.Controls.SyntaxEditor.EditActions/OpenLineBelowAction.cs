...
public class OpenLineBelowAction : EditActionBase
{
	internal static OpenLineBelowAction ikD;

	public OpenLineBelowAction()
	{
		grA.DaB7cz();
		base._002Ector(SR.GetString(SRName.UICommandOpenLineBelowText));
	}
	...
	public override void Execute(IEditorView view)
	{
		...
		using (view.Selection.CreateBatch(EditorViewSelectionBatchOptions.None))
		{
			...
			{
				TextPosition lastPosition = array[num2].LastPosition;
				TextPosition endPosition = view.GetViewLine(lastPosition).EndPosition;
				if (endPosition != textPosition)
				{
			...
			for (int num3 = list.Count - 1; num3 >= 0; num3--)
			{
				...
				if (indentProvider == null || indentProvider.Mode != IndentMode.None)
				{
					...
					{
						num4 = Math.Max(0, indentProvider.GetIndentAmount(new TextSnapshotOffset(currentSnapshot, offset), num4));
					}
					text = StringHelper.GetIndentText(document.AutoConvertTabsToSpaces, document.TabSize, num4);
				}
				array2[num3] = new TextPositionRange(new TextPosition(position.Line + num3 + 1, text?.Length ?? 0));
				textChange.InsertText(offset, Q7Z.tqM(7894) + text);
			}
	...
	internal static bool AkB()
	{
		return ikD == null;
	}

	internal static OpenLineBelowAction Gk0()
	{
		return ikD;
	}
}
